# run: python processNoise.py -i /data/exp/IceCube/2022/unbiased/surface/V6/sae_data/SAE_data_136***_1_IT.i3.gz -f "2022_136" -o "output_processed_noise_136_nores.i3"

#This script filters the soft triggered events; filters the non-cascaded mode; applies the module "RemoveTAXIArtifacts"; applies the module "RemoveSpikes" (to correct the high peaks that were not removed) --it helps a lot but doesn't remove ALL spikes--; aplies other modules; calculates the SNR and creates a key with a list of 3 values for each frame (the 3 values correspond one to each antenna).
#Its output is an I3file.

#It does not apply the module "TraceResampler" because in the script "selectionByConsecutiveBins.py" the consecutive bins are compared.
...
class RemoveSpikes(icetray.I3Module):
  def __init__(self,ctx):
    icetray.I3Module.__init__(self,ctx)       
    self.inputName = ""
    self.AddParameter("InputName", "Input antenna data map", self.inputName)   
    self.outputName = ""
    self.AddParameter("OutputName", "Output antenna data map", self.inputName)
  def Configure(self):
  ...
  def DAQ(self,frame):
    ...
    for iant, antkey in enumerate(antDataMap.keys()):
        ...
        for ichannel, chkey in enumerate(channelMap.keys()): 
            fft = dataclasses.FFTData(channelMap[chkey].GetFFTData()) 
            timeSeries = fft.GetTimeSeries()    
            RMSValue = radcube.GetRMS(timeSeries)
            t, A = radcube.RadTraceToPythonList(timeSeries)            
            for i in range(len(A)):  #compares consecutive bins to detect Spikes
                if i<=1021:
                    if (abs(A[i+1])<abs(A[i])/1.06 or abs(A[i+1])>abs(A[i])*1.06) and (abs(A[i+1])<abs(A[i+2])/1.06 or abs(A[i+1])>abs(A[i+2])*1.06):                        
                        A[i+1]=(A[i]+A[i+2])/2
                    
            fft.LoadTimeSeries(radcube.PythonListToRadTrace(t, A))
            outputChMap[chkey] = dataclasses.I3AntennaChannel(fft)
...
tray.AddModule(RemoveSpikes, "SpikesRemoved",
               InputName="ArtifactsRemoved",
               OutputName="SpikesRemoved"
              )
